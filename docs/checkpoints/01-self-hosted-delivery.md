...
Run `pactwright lifecycle status`, `next` and `run` on the fixture consumer at each lineage position. `run` must stop cleanly at the configured Gate, at manual entries and, for capability-backed automatic responsibilities, with the missing-invoker execution failure; it must not simulate any output.
