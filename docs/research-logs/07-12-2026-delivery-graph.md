# Claude Code Implementation Outline: GitHub-Native AI Software Delivery (v3.1)

Revision note (v3.1): this version folds in the Phase 9.5 delta — binding mix-and-match amendments in the proposal market, a lane market pairing each lane with an owning agent through per-lane catalog files, lane execution mirrored as native GitHub issues (a one-way view), the conveyor (every lifecycle command prints its paste-ready next command), navigation views, and the role-to-implementation mapping. Sections 4, 7, 8, 11, 13, 16, 20, 21, 24, and 27 updated; the build order gains Phase 9.5 and a guard-rail-hardening Step 0 at the head of the Projects phase.
...
* `owner` — optional on `brief` from the lane-market phase; the agent selected to own the lane, recorded by decomposition
...
Merging parts of candidates has defined, binding mechanics. The decision body names a base candidate, grafts from named siblings, and mandatory fixes; these amendments are binding, and the effective contract is the approved contract plus its selecting decision's amendments — the approved contract body is never edited. Decomposition carries the amendments into the briefs, and every review judged against the approved contract — contract review, integration review, and drift review included — reads the contract and its selecting decision together. An amendment that changes the intended behaviour returns to human approval under the scope-integrity rules.
...
Each lane is paired with an owning agent through a lane market. One catalog file per lane (markdown, beside the agent files) declares in frontmatter the agents eligible to own it and a default; decomposition weighs the eligible agents against the brief's scope, picks the default unless it states a reason, records the chosen owner on the lane brief, and shows the pairing in its wave plan. Eligibility is the lane's policy, not the agent's claim — agent files never declare lanes, and granting a lane means editing that lane's catalog file, so the diff is the authorization change. The test-verification lane is owned by the Test Writer, never an implementation agent; a drift test pins the catalog files, the operating-instructions lane table, the schema's lane keys, agent-name resolution, default-in-eligible membership, and the test-verification pairing together, so separation of duties is enforced mechanically, not by prose.
...
Role-to-implementation mapping: not every role is a standalone agent file. The Decomposer runs as the brief and lane-decomposition commands; the Code Archaeologist runs as the context-loading step inside implementation. Implementation Agents are realized as one implementer agent per lane — selected through the lane market — plus the Test Writer for verification. The roles below describe responsibilities; the lane catalog and command files describe where each one executes.
...
* That the effective contract is the approved contract plus its selecting decision's binding amendments
* How the conveyor works: every lifecycle command's closing report ends with the paste-ready next command(s), which must satisfy the standing rules for the change's class — the conveyor prints, it never executes
* The output-attention conventions: decision-first reports and essential-first node bodies, as guidance rather than validation gates
...
Lane execution is visible before the board exists. A `spec:issue-sync` script mirrors the graph into native GitHub issues, one direction only: one issue per lane brief; one parent issue per multi-lane contract with the lanes as native sub-issues; blocked-by relations from the wave plan. Issues follow the board's rule — the graph is truth, the issues are a view: an issue links to its node by an embedded node-id marker (the graph never references the view), hand-edits are corrected on the next sync, a lane issue closes when its brief's evidence goes final, and the parent closes at final integration. Board items for lane briefs are these issues; the board attaches them rather than minting duplicate draft items.
...
* `issue-sync.yml` — reconciles the lane-issue view on merge to main and manual dispatch; one-direction, warn-only — a sync failure annotates, never blocks.
...
* Release checks
* `spec:project-sync`
* `spec:issue-sync` — the lane-issue view
* `spec:status` — read-only: per live intent, the lifecycle stage and the paste-ready next command, so the journey survives a lost session; `spec:project-sync` reuses its derivation
* Navigation views — `spec:index` additionally emits `trails.md` (one section per intent: its contracts, comparison, decision, briefs with lanes and owners, evidence, integration) and `status.md` (the open-work view), so the graph is followable without tooling
...
**Phase 9.5 — Conveyor, lane market & issue-tracked lanes.** Every command's closing report prints the paste-ready next command; per-lane catalog files pair each lane with an owning agent (the optional `owner` field on brief is the one schema touch — backfill-free); binding mix-and-match amendments formalized; lane execution mirrored as native issues with sub-issues and blocked-by relations; `spec:status` and the `trails.md`/`status.md` navigation views; output-attention conventions.

10. **Projects view (lane & market visibility)** — opens with a guard-rail-hardening Step 0 (decision-to-intent provenance and the unbacked-addressed guard, the class-3 lane-decomposition rule, synthesis-parentage hardening, malformed-cutoff validation — each with dated cutoffs per the migration discipline), then one-way board sync with lane and market fields and views; board items for lane briefs are the Phase 9.5 issues.
...
* A lane market pairing each lane with an owning agent through per-lane catalog files
* Native GitHub issues as a one-way view of lane execution, and a conveyor of paste-ready next commands
* Evidence nodes
* Drift findings
* Override nodes
