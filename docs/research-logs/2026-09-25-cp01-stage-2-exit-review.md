...
|---|---|---|---|
| X1 | S09/R05 rule 12; S06/R04 | Rule 12 failed current Evidence whose Brief has no execution-state document, which includes this repository's two 0.0.1 Evidence records (Step 25) and every migrated 0.0.1 project (S19/AC07); Evidence is immutable, so validate could never pass. | **Owner decision:** rule 12 applies only when the Brief has a document; Evidence without one is reported as unavailable verification and does not fail. Core v4 §57 states it; S09/R05, R11 and AC11 `legacy-evidence-without-state`. |
...
**Checkpoint 1 Stage 2 exit review v1**
